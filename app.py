...
# 🤖 [版本號] v19.1 
BOT_VERSION = "v19.1 (導入gemini-3.6)"
...
def handle_message(event):
    ...
    if msg in ["選股邏輯", "推薦說明", "篩選條件","右側邏輯"]:
        logic_text = (
            ...
            " ‧ 剔除低價股：股價必須 > 10 元。\n"
            " ‧ 資金熱區：單日成交金額必須 > 1 億元且當日收紅。\n\n"
            "2️⃣ 第二關：基本面與大戶籌碼 (勝率核心)\n"
            " ‧ 營收真成長：營收 YoY 必須 > 10%。\n"
            " ‧ 大戶共識：近 5 日「外資+投信」買超合計 > 3 億元。\n\n"
            "3️⃣ 第三關：避雷針與過熱風控\n"
            " ‧ 假突破淘汰：若上影線大於實體 1.5 倍，絕對淘汰。\n"
            " ‧ 過熱降權：若月線乖離 > 15%，打上「🔥過熱妖股」標籤並扣減動能分數，防追高。\n"
            "────────────────\n"
            "💡 常見問答：為何強勢股偶爾會「漏網」？\n"
            "1. 結構風險：僅選「上市櫃」標的。\n"
            "2. 純題材炒作：最新營收年增率未達 10%，缺乏業績支撐。\n"
            "3. 缺乏法人背書：法人買超未達 3 億，籌碼相對鬆散。\n"
            "📌 鐵律：只推薦有「基本面」與「大資金」雙重背書的優質標的！\n"
            "────────────────\n"
            "🎯 【右側動能股：無痕停利損模型】\n"
            " ‧ 上榜超過 30 天自動除名。\n"
            " ‧ 累積獲利達 +20% (停利) 或 跌破 -8% (停損) 時自動從追蹤清單剃除。"
        )
    ...
    if msg in ["左側邏輯", "左側說明", "左側條件", "黃金坑邏輯"]:
        left_logic_text = (
            ...
            " ‧ 跌深委屈：股價必須在季線下方 (負乖離)。\n"
            " ‧ 量縮窒息：今日成交量低於 20日均量 90%。\n"
            " ‧ 低波築底：近 10 日振幅 < 15% (未劇烈下殺)。\n"
            " ‧ 尚未起漲：近 5 日漲幅 < 8% (買在安全點)。\n\n"
            "3️⃣ 第三關：籌碼定錨與智能轉機漏斗\n"
            " ‧ 大戶試單：近 5 日內法人至少買超 1 天，且買超金額/佔比達標。\n"
            " ‧ 智能轉機安檢：若 EPS < 0 (虧損)，必須同時滿足「大資金反轉K線」、「籌碼高度集中」與「營收 YoY > 0」，缺一不可，否則淘汰。\n"
            "────────────────\n"
            "🎯【獨家信心評分系統】\n"
            "達標後，依據「法人連買天數」、「極致量縮 (<50%)」、「負乖離深度」、「底部型態(吞噬/槌子線)」、「RSI 勾頭」給予綜合評分，大於 55 分才發車！\n"
            "────────────────\n"
            "🎯 【左側黃金坑：無痕停利損模型】\n"
            " ‧ 上榜超過 30 天自動除名。\n"
            " ‧ 坑被填平獲利達 +15% (停利) 或 接刀失敗破底 -10% (停損) 時自動從追蹤清單剃除。"
        )
