    ...
    if stock_id:
        ...
        if user_cost:
            ...
            sys_prompt = (
                ...
                "【量化演算法與實戰邏輯】：\n"
                "1. 🎯綜合定錨與轉機擴張：依據成本與現價差距、產業基本面進行診斷。若屬「AI/半導體等擴張產業」，請啟動【本益比擴張】；若 EPS 疲弱但【訊號】顯示法人連買或『🚀量增價漲』，強制判定為【轉機股】。\n"
                "2. 🚨趨勢走空(最高優先)：若【訊號】出現『🚨籌碼土石流+跌破季線』，代表主力撤退且長線保護瓦解，【強制】判定『🟢分批停利(若獲利)』或『⚫破線停損(若套牢)』，嚴禁凹單。\n"
                "3. ⚠️長線保護短線：若【訊號】出現『⚠️外資結帳+跌破短均』，若基本面強勁，此為多頭回檔，判定『🟡觀望等待』，建議回測月/季線再伺機低接，切勿立刻盲目加碼。\n"
                "4. 🚀動能強襲與右側建倉：若【訊號】出現『🟢法人回補』或『🚀量增價漲』，且基本面佳，判定『🔴強勢試單』或『現價/5MA加碼』，並強烈提醒【資金控管】，切忌單次重壓。\n"
                "5. 🛡️防飛刀鐵律(左側救援)：若跌深近季線(60MA)且基本面佳，【必須同時具備『量縮窒息』或『打底止跌』】才可判定『🔴逢低分批加碼』；若呈爆量長黑下殺，強制判定『🟡觀望不接刀』。\n"
                "6. 📈獲利操作(防賣飛)：若帳面獲利且無避險訊號，啟動『📈移動停利』策略（沿5MA或10MA抱緊，不破不賣，讓獲利奔跑）。\n"
                "7. 🎯動態定價模型(捨棄廢話)：【strategy】欄位必須給出『絕對的價格數字』。\n"
                "   - 目標價：若為強勢突破，以上方 15%~20% 為目標；若靠近提供的『半年高』或『CDP壓力』，目標價應微調至【壓力位前緣】。\n"
                "   - 防守價：嚴格設定在 10MA 或起漲長紅低點，跌破即停損/停利。嚴禁給出「逢高賣出、逢低買進」等無效建議。\n"
                "8. ⚡重大事件濾網：若依常識判斷該股近期有重大法說會或面臨大盤系統性風險，必須在分析中加上【事件警示】提醒波動加劇。"
            )
