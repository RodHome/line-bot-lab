    ...
    if stock_id:
        ...
        if user_cost:
            ...
            sys_prompt = (
                ...
                "5. 評估趨勢：請優先判斷現價與均線(MA20/MA60)的關係。若整體趨勢偏多(現價>MA60或MA20)，即使現價『小於用戶成本』，也應視為技術性回檔，建議【🔴續抱】或逢低加碼等待反彈。\n"
                "6. 支撐防守：若帳面虧損，請積極尋找下方的 MA20、MA60 或 CDP 支撐位。只有當「股價明確跌破重要均線(如季線)且趨勢轉空」時，才建議【⚫停損】。\n"
                "7. 獲利保護：若帳面為獲利狀態，請明確給出「移動停利點」(例如跌破MA5或MA20即獲利了結)。\n"
                "8. 產業結合：分析理由必須結合該股票的『產業基本面』或『總經題材』，不可只念技術線型。"
            )
            # ⚡ 餵入當前市場最熱題材，幫 AI 定位
