...
def call_gemini_json(prompt, system_instruction=None, schema=None):
    ...
    target_models = ["gemini-3.8-flash", "gemini-3.5-flash", "gemini-3.5-flash-lite", "gemini-2.5-flash"]
    final_prompt = prompt 
...
def handle_message(event):
    ...
    if msg == "推薦" or msg == "選股":
        ...
        line_bot_api.reply_message(event.reply_token, FlexSendMessage(alt_text="請選擇選股分類", contents=menu_flex))
        return
    ...
    if msg.startswith("推薦 ") or msg.startswith("選股 "):
       ...
       # 🔥 優化：更精確的回報找不到標的之原因
        if not good_stocks:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"⚠️ 今日「{choice}」中暫無符合條件的標的。"))
            return
        ...
        try:
            raw_data = requests.get("https://raw.githubusercontent.com/RodHome/line-bot-lab/main/daily_recommendations.json", headers={'Cache-Control': 'no-cache'}, timeout=5).json()
            downgraded = []
            tw_now = datetime.now(timezone.utc) + timedelta(hours=8)
            for s in raw_data:
                if "權值" in target_type and s.get('cap_size') != "大型權值股": continue
                if "中小" in target_type and s.get('cap_size') != "中小型股": continue
                
                rank = s.get('capital_rank', 'C')
                if rank in ['B', 'C']:
                    try:
                        f_date = datetime.strptime(s.get('first_entry_date', ''), '%Y-%m-%d').replace(tzinfo=timezone(timedelta(hours=8)))
                        if (tw_now - f_date).days <= 15: # 15天內推薦過
                            downgraded.append(s)
                    except: pass
            
            if downgraded:
                # 依危險程度排序 (C級最危險)
                downgraded.sort(key=lambda x: (1 if x.get('capital_rank')=='C' else 2, x.get('date', '')))
                warn_contents = [
                    {"type": "text", "text": "⚠️ 歷史庫存降級警示", "weight": "bold", "size": "md", "color": "#ffffff"},
                    {"type": "text", "text": "過去15天曾入榜，今日已轉弱或過熱，請留意出場時機：", "size": "xs", "color": "#ffcccc", "wrap": True, "margin": "sm"},
                    {"type": "separator", "margin": "md", "color": "#ff9999"}
                ]
                for ds in downgraded[:5]: # 考量版面，最多顯示5檔最危險的
                    r_txt = "🔴 破線出貨" if ds.get('capital_rank')=='C' else "🟡 爆量過熱"
                    warn_contents.append({
                        "type": "box", "layout": "horizontal", "margin": "sm",
                        "contents": [
                            {"type": "text", "text": f"{ds['name']}({ds['code']})", "size": "sm", "color": "#ffffff", "weight": "bold", "flex": 3},
                            {"type": "text", "text": r_txt, "size": "xs", "color": "#ffb3b3", "align": "end", "flex": 2}
                        ]
                    })
                ...
                warn_bubble = {
                    "type": "bubble", "size": "hecto",
                    "body": {
                        "type": "box", "layout": "vertical", "backgroundColor": "#D32F2F", "spacing": "sm",
                        "contents": warn_contents
                    },
                    "footer": {
                        "type": "box", "layout": "vertical", "backgroundColor": "#D32F2F",
                        "contents": [
                            {"type": "button", "action": {"type": "message", "label": "💡 點我一鍵持股診斷", "text": "如何診斷"}, "style": "primary", "color": "#b71c1c", "height": "sm"}
                        ]
                    }
                }
   ...
   # ==========================================
    ...
    if stock_id:
        ...
        # ==========================================
        # 🆕 新增：起漲 vs 過熱判定量化特徵 (直接插在這裡)
        # ==========================================
        raw_closes = data.get('raw_closes', [])
        prev_close = raw_closes[-2] if len(raw_closes) > 1 else data['close']
        daily_spread = round((data['close'] - prev_close) / prev_close * 100, 1) if prev_close > 0 else 0
        
        ma20_val = data.get('ma20', 0)
        ma60_val = data.get('ma60', 0)
        ma_gap = round((ma20_val - ma60_val) / ma60_val * 100, 1) if ma60_val > 0 else 0
        k_mid = round((data.get('open', data['close']) + data['close']) / 2, 2)
        # ==========================================
        ...
        if user_cost:
            ...
            sys_prompt = (
                ...
                "【輸出鐵律】：\n"
                "直接給出『洞察』。絕對禁止重複報價、念出帳面虧損數字，或給出模稜兩可的廢話。\n"
                "【量化演算法與實戰邏輯】：\n"
                "1. 🎯綜合定錨與轉機擴張：依據成本與現價、產業面進行診斷。若屬「AI/半導體等擴張產業」，啟動【本益比擴張】；若 EPS 疲弱但【訊號】顯示法人連買或『🚀量增價漲』，【強制】判定為【轉機股】，其位階等同於基本面佳。\n"
                "2. 🚨趨勢走空與過熱(最高優先)：若【訊號】出現『🚨籌碼土石流+跌破季線』，或已跌破10MA且伴隨『爆量過熱』，代表主力撤退或高檔出貨，【強制】判定『🟢停利/減碼出場』或『⚫破線停損』，嚴禁凹單。\n"
                "3. ⚠️長線保護短線：若【訊號】出現『⚠️外資結帳+跌破短均』，若基本面強勁，此為多頭回檔，判定『🟡觀望等待』，建議回測月/季線伺機低接。\n"
                "4. 🚀動能強襲與防追高分流：若出現『🟢法人回補』或『🚀量增價漲』，請依據【均線發散度】與【當日漲幅】嚴格分流：\n"
                "   - 💡【初升段突破】：若發散度 < 10% 且出量長紅，判定為『🔴強勢加碼』。但【絕對禁止】現價追買，進場策略強制寫明在【紅K中值】與【5MA】區間量縮承接。\n"
                "   - 🚨【高檔過熱出貨】：若發散度 > 15% 且單日漲幅 > 5%，判定為『🟡觀望等待』或『🟢分批停利』。強制警告「長線發散且乖離過大，嚴防主力出貨，嚴禁追高」。\n"
                "5. 🛡️防飛刀鐵律(左側救援)：若跌深近季線(60MA)且基本面佳，【必須同時具備『量縮窒息』或『打底止跌』】才判定『🔴逢低分批加碼』；若呈爆量長黑下殺，強制判定『🟡觀望不接刀』。\n"
                "6. 📈獲利操作與存股鐵律：若帳面獲利且無避險訊號，啟動『📈移動停利』策略（沿5MA或10MA抱緊，不破不賣）。若為定存股且現價大漲導致「殖利率 < 4%」，強制給出『🔄 獲利換股』，失去高息保護傘即離場。\n"
                "7. 🎯動態定價模型(捨棄廢話)：【strategy】欄位必須給出『絕對的價格數字』。\n"
                "   - 目標價：若為轉機或強勢突破，無視短壓，直接以上方 15%~20% 或前波高點為目標。\n"
                "   - 防守價：嚴格設定在 10MA、月線或季線等關鍵支撐。\n"
                "   - ⚠️【名詞鐵律】：若你建議的防守價數字『低於』使用者成本，請務必寫『停損』；若防守價『高於』使用者成本，才可寫『停利』或『保護獲利』。絕不可在跌破成本的價位叫使用者停利！\n"
                "8. ⚡重大事件濾網：若依常識判斷該股近期有重大法說會或面臨大盤風險，必須在分析中加上【事件警示】。"
            )

            # 🔥 補上 10MA
            user_prompt = f"標的:{name}(產業:{sector}), 現價:{data['close']}, 當日漲幅:{daily_spread}%, 紅K中值:{k_mid}, 均線發散度(20MA與60MA差距):{ma_gap}%, EPS:{eps}, 【今日籌碼】(外資:{ai_today_f}/投信:{ai_today_t}), 【5日累積】(外資:{ai_af_val}/投信:{ai_at_val}), MA5:{data['ma5']}, MA10:{data.get('ma10',0)}, MA20:{data['ma20']}, MA60:{data['ma60']}, CDP壓力:{cdp_res}, CDP支撐:{cdp_sup}, 半年高:{six_m_high}, 半年低:{six_m_low}, 訊號:{signal_str}"
