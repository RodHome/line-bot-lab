    ...
    if stock_id:
        ...
        if user_cost:
            ...
            sys_prompt = (
                ...
                "【實戰判斷邏輯】：\n"
                "1. 資金控管：依據使用者的『成本與現價差距』(判斷套牢深度或獲利水位)，結合『產業基本面(EPS/殖利率)』進行無情診斷。\n"
                "2. 套牢救援：若屬主流趨勢或基本面強勁(EPS高)，技術面跌深且接近季線(60MA)即是『🔴逢低加碼』攤平良機；若基本面極爛(EPS差/無殖利率)且跌破均線，直接給予『⚫破線停損』，嚴禁留戀。\n"
                "3. 獲利操作：若處於獲利狀態且技術面過熱(RSI高/乖離大)，適時給予『🟢獲利了結』或『移動停利』建議。\n"
                "4. 具體定價：無論加碼或防守，【strategy】中必須給出『絕對的價格數字』(參考均線或前低/前高)。"
            )
            
            # 🔥 升級火力：把 EPS、殖利率與使用者的帳面盈虧(%) 全部餵給 AI！
            user_prompt = f"標的:{name}(產業:{sector}), 現價:{data['close']}, 使用者成本:{user_cost} (帳面盈虧:{profit_pct}%), 基本面(EPS:{eps}/殖利率:{yield_rate}), 均線(5MA:{data['ma5']}/60MA:{data['ma60']}), 訊號:{signal_str}"
            
            # 🔥 重新定義 Schema：解除字數限制，逼迫它使用專業術語
            cost_schema = {
                "type": "OBJECT",
                "properties": {
                    "action": {"type": "STRING", "description": "必須精準輸出此四者之一：🔴逢低加碼 / 🟡續抱觀察 / 🟢獲利了結 / ⚫破線停損"},
                    "analysis": {"type": "STRING", "description": "冷酷無情的綜合診斷：基於使用者的帳面盈虧狀態，結合基本面(EPS/殖利率)與技術面給出真實研判，請使用專業術語，精簡但具備高資訊密度。"},
                    "strategy": {"type": "STRING", "description": "行動準則與明確點位：給出具體的「防守價(停損點)」或「加碼價(攤平點)」數字，以及後續操作紀律。"}
                },
