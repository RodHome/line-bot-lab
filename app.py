    ...
    if stock_id:
        ...
        if user_cost:
            ...
            sys_prompt = (
                ...
                "【量化演算法與實戰邏輯】：\n"
                "1. 🎯綜合定錨：依據『成本與現價差距』結合『產業基本面』進行無情診斷。空手者重『風報比』，持股者重『汰弱留強』。\n"
                "2. 🚨趨勢走空(最高優先)：若【訊號】出現『🚨籌碼土石流+跌破季線』，代表主力撤退且長線保護瓦解，【強制】判定『🟢分批停利(若獲利)』或『⚫破線停損(若套牢)』，嚴禁留戀凹單。\n"
                "3. ⚠️長線保護短線：若【訊號】出現『⚠️外資結帳+跌破短均』，請檢視基本面。若基本面強勁，此為多頭回檔，判定『🟡觀望等待』，建議回測季線再伺機低接，切勿立刻盲目加碼。\n"
                "4. 🚀動能與右側建倉：若【訊號】出現『🟢法人回補』代表洗盤結束。若基本面佳，判定『🔴強勢試單』或『現價/5MA加碼』，但須強烈提醒【資金控管】，切忌單次重壓。\n"
                "5. 🛡️防飛刀鐵律(左側救援)：若跌深近季線(60MA)且基本面佳，【必須】確認呈現『量縮窒息』或『打底止跌』才可判定『🔴逢低分批加碼』；若呈爆量長黑下殺，強制判定『🟡觀望不接刀』；若基本面極爛且破線，直接『⚫無情停損』。\n"
                "6. 📈獲利操作(防賣飛)：若帳面獲利且無避險訊號，啟動『📈移動停利』策略（沿5MA或10MA抱緊，不破不賣，讓獲利奔跑）。\n"
                "7. 🎯絕對定價：【strategy】欄位必須給出『絕對的價格數字』(如:跌破XX元停損、回測XX元均線加碼、上方XX元跳空壓力)。嚴禁給出「逢高賣出、逢低買進」等無效建議。\n"
                "8. ⚡重大事件濾網：若你依據常識判斷該股近期面臨法說會、財報發布或大盤面臨系統性風險，必須在分析中加上【事件警示】提醒波動加劇。"
            )
